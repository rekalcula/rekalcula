...
export default function AdvisorPage() {
  ...
  const cargarAnalisisGuardados = async () => {
    ...
    try {
      ...
      console.error('Error cargando anlisis:', err)
    } finally {
  ...
  const generarNuevoAnalisis = async () => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Error generando anlisis')
      }

      setAnalisisActual(result)
    } catch (err: any) {
      setError(err.message || 'Error generando anlisis')
    } finally {
      setGenerando(false)
    }
  }
  ...
  const borrarSeleccionados = async () => {
    ...
    if (!confirm(`Eliminar ${seleccionados.size} anlisis?`)) return
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 bg-[#262626] min-h-screen">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xl text-[#FFFCFF] mt-1">Genera y guarda anlisis de tu negocio
          </p>
        </div>
      </div>
      ...
      {tabActiva === 'nuevo' && (
        <div className="space-y-6">
          <div className="bg-[#262626] rounded-xl p-4 sm:p-6">
            <h2 className="text-lg font-semibold text-[#FFFCFF] mb-4">Configurar Anlisis</h2>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold text-[#FFFCFF]">
                      Anlisis Generado - {traducirSector(analisisActual.sector)}
                    </h3>
                  ...
                  >
                    {guardando ? 'Guardando...' : ' Guardar Anlisis'}
                  </button>
                </div>
              </div>
          ...
          {analisisActual?.sinRecomendaciones && (
            <div className="bg-[#262626] rounded-xl p-8 text-center">
              ...
              <p className="text-xl text-[#ACACAC]">
                <strong>Requisito mnimo:</strong> Se necesitan al menos <strong>15 das de datos de ventas</strong> para realizar un anlisis cientficamente vlido y detectar tendencias significativas.
              </p>
              </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          {cargandoGuardados ? (
            <div className="bg-[#262626] rounded-xl p-8 text-center">
              ...
              <p className="text-[#ACACAC] mt-4">Cargando anlisis...</p>
            </div>
          ) : analisisGuardados.length === 0 ? (
            <div className="bg-[#262626] rounded-xl p-8 text-center">
              <IconFolder />
              <h3 className="text-lg font-medium text-[#FFFCFF] mt-4 mb-2">Sin anlisis guardados</h3>
              <p className="text-[#ACACAC] text-[20px] mb-4">Genera tu primer anlisis para empezar</p>
              <button
                onClick={() => setTabActiva('nuevo')}
                className="px-6 py-2 bg-[#0d0d0d] text-white rounded-lg hover:bg-[#2d2d2d] transition-colors font-medium"
              >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div>
                          <h3 className="font-medium text-[#FFFCFF]">
                            Anlisis #{analisisGuardados.length - analisisGuardados.indexOf(analisis)}
                          </h3>
                          <p className="text-xl text-[#ACACAC]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold text-[#FFFCFF]">
                  Anlisis del {formatearFecha(analisisDetalle.created_at)}
                </h2>
