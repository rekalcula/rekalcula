  ...
  try {
    ...
    if (compare) {
      const { data: prevData, error: prevError } = await supabase
        ...
        .lt('sale_date', prevEndDate.toISOString().split('T')[0])
    ...
    // Agregar datos por producto - período actual
    ...
    for (const sale of currentSales || []) {
      for (const item of sale.sale_items || []) {
        const productName = item.product_name?.toLowerCase().trim() || 'Otros'
    ...
    // Agregar datos por producto - período anterior
...
// Normalizar nombres de productos similares
function normalizeProductName(name: string): string {
  const normalized = name.toLowerCase().trim()
  
  // Agrupar variaciones comunes
  if (normalized.includes('café') || normalized.includes('cafe')) {
    if (normalized.includes('solo')) return 'café solo'
    if (normalized.includes('leche') || normalized.includes('cortado')) return 'café con leche'
    return 'café'
  }
  if (normalized.includes('desayuno')) return 'desayuno'
  if (normalized.includes('tostada')) return 'tostada'
  if (normalized.includes('zumo')) return 'zumo'
  if (normalized.includes('agua')) return 'agua'
  if (normalized.includes('refresco') || normalized.includes('coca')) return 'refresco'
  if (normalized.includes('cerveza') || normalized.includes('caña')) return 'cerveza'
  if (normalized.includes('vino')) return 'vino'
  if (normalized.includes('bocadillo') || normalized.includes('sandwich')) return 'bocadillo'
