...
export default function CostAnalyzer() {
  ...
  const [realCostsStats, setRealCostsStats] = useState<RealCostStats[]>([])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch real costs from usage logs
      const realCostsRes = await fetch('/api/admin/ai-usage-stats')
      const realCostsData = await realCostsRes.json()
      if (realCostsData.success && realCostsData.stats) {
        setRealCostsStats(realCostsData.stats)
        
        const realCostsMap: any = {
          invoice: null,
          ticket: null,
          analysis: null
        }

        realCostsData.stats.forEach((stat: RealCostStats) => {
          if (stat.operation_type === 'invoice') realCostsMap.invoice = stat.avg_cost_eur
          if (stat.operation_type === 'ticket') realCostsMap.ticket = stat.avg_cost_eur
          if (stat.operation_type === 'analysis') realCostsMap.analysis = stat.avg_cost_eur
        })

        setRealCosts(realCostsMap)
        setHasRealData(realCostsData.stats.length > 0)

        // Auto-update costs if real data exists and recommendations are available
        if (realCostsData.recommendations && realCostsData.stats.length > 0) {
          // Don't auto-update, just show the suggestions
        }
      }
  ...
  const applyRecommendedCosts = () => {
    if (!hasRealData) return

    const buffer = 1.2 // 20% buffer
    setCustomCosts({
      invoice: realCosts.invoice ? Math.ceil(realCosts.invoice * buffer * 1000) / 1000 : customCosts.invoice,
      ticket: realCosts.ticket ? Math.ceil(realCosts.ticket * buffer * 1000) / 1000 : customCosts.ticket,
      analysis: realCosts.analysis ? Math.ceil(realCosts.analysis * buffer * 1000) / 1000 : customCosts.analysis,
    })
    setMessage({ type: 'success', text: 'Costos recomendados aplicados (real + 20% buffer)' })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Real Costs vs Configured Costs */}
      {hasRealData && (
        <div className="bg-gradient-to-br from-green-900/20 to-blue-900/20 rounded-xl p-6 border border-green-700/50">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Activity className="w-5 h-5 text-green-400" />
              <h3 className="text-lg font-semibold text-white">Costos Reales vs Configurados</h3>
            </div>
            <button
              onClick={applyRecommendedCosts}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Invoice */}
            <RealVsConfiguredCard
              type="invoice"
              label="Facturas"
              icon={FileText}
              realCost={realCosts.invoice}
              configuredCost={customCosts.invoice}
              stats={realCostsStats.find(s => s.operation_type === 'invoice')}
            />
            ...
            <RealVsConfiguredCard
              ...
              label="Tickets"
              icon={Receipt}
              realCost={realCosts.ticket}
              configuredCost={customCosts.ticket}
              stats={realCostsStats.find(s => s.operation_type === 'ticket')}
            />
            ...
            <RealVsConfiguredCard
              ...
              label="Análisis"
              icon={Lightbulb}
              realCost={realCosts.analysis}
              configuredCost={customCosts.analysis}
              stats={realCostsStats.find(s => s.operation_type === 'analysis')}
            />
          </div>
        </div>
      )}
      ...
      {!hasRealData && (
        <div className="bg-yellow-900/20 border border-yellow-700 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-yellow-300 font-medium">No hay datos reales de uso todavía</p>
            <p className="text-yellow-400/80 text-sm mt-1">
              Procesa algunas facturas, tickets o análisis para que el sistema calcule los costos reales automáticamente.
              Necesitas al menos 10-20 operaciones para obtener promedios confiables.
            </p>
          </div>
        </div>
      )}
...
// Real vs Configured Card Component
function RealVsConfiguredCard({
  type,
  label,
  icon: Icon,
  realCost,
  ...
  const recommendedCost = realCost ? Math.ceil(realCost * 1.2 * 1000) / 1000 : configuredCost
  const difference = realCost ? ((configuredCost - realCost) / realCost) * 100 : 0
  const isOverpriced = difference > 30
  const isUnderpriced = difference < -10
    ...
    <div className="bg-[#1a1a1a] rounded-lg p-4 border border-gray-700">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          <span className="text-gray-400 text-sm">Costo Real:</span>
          <span className="text-green-400 font-bold">
            {realCost ? `€${realCost.toFixed(4)}` : 'N/A'}
          </span>
        </div>

        {/* Configured Cost */}
        <div className="flex justify-between items-center">
          <span className="text-gray-400 text-sm">Configurado:</span>
          <span className={`font-bold ${
            isOverpriced ? 'text-yellow-400' : isUnderpriced ? 'text-red-400' : 'text-white'
          }`}>
            €{configuredCost.toFixed(4)}
          </span>
        </div>
        ...
        {stats && (
          <div className="pt-2 border-t border-gray-700 text-xs text-gray-500">
            <div className="flex justify-between">
              ...
              <span>{stats.total_operations}</span>
            </div>
            <div className="flex justify-between">
              <span>Rango:</span>
              <span>€{stats.min_cost_eur.toFixed(4)} - €{stats.max_cost_eur.toFixed(4)}</span>
            </div>
          </div>
        )}
      </div>
...
// Plan Cost Card Component (mantener igual)
...
// Package Cost Card Component (mantener igual)
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">{config.label} • {pkg.amount} créditos</p>
          </div>
        </div>
        <div className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${
          analysis.isProfitable 
      ...
      <div className="bg-[#1a1a1a] rounded-lg p-4">
        <div className="grid grid-cols-5 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-blue-400 font-bold">€{(pkg.price / pkg.amount).toFixed(3)}</p>
          </div>
          <div>
